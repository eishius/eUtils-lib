...
function nameoffunction()
    eUtils:dataLoad(self:SteamID(), "NErd", "players" true )
end
```
**Using a global**
```lua
local a = eUtils.Debug
a = true
```

### Examples
**Function: dataSave( id, path, name, data, bool )**
```lua
function Example( sid, p )
    if p then 
        sid = p:SteamID()
    end 
    return eUtils:dataLoad( sid, "Example.path", "Example.name", "Example.data", true)
end 

--[[
    The way this function works is it grabs a steamid 32 and loads the data that was saved using the function eUtils:dataSave
]]

```

**Function: dataLoad( id, path, name, data, bool )**
```lua
function Example( sid, p )
    local data = "u stink" .. p:SteamID64()
    if p then 
        sid = p:SteamID()
    end 
    return eUtils:dataSave( sid, "Example.path", "Example.name",  data, true)
end 

--[[
    the way this function works is it grabs a steamid formats it makes a path makes a file off of the formated steamid and saves the data given  
]]
